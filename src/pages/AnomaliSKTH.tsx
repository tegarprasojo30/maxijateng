...
import { fetchAnomaliSKTH, submitKonfirmAnomaliSKTH, type AnomaliSKTH } from "@/lib/sheets";
...
export default function AnomaliSKTHPage() {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const { data: result, isLoading } = useQuery({
    queryKey: ["anomaliSKTH"],
    queryFn: fetchAnomaliSKTH,
    staleTime: 5 * 60 * 1000,
  });
  ...
  const [activeRow, setActiveRow] = useState<AnomaliSKTH | null>(null);
  ...
  const filtered = useMemo(() => {
    ...
    return r;
  }, [anomaliData, kabFilter, jenisFilter, search]);

  const totalPages = Math.max(1, Math.ceil(filtered.length / PAGE_SIZE));
  const paged = filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);

  useEffect(() => { setPage(1); }, [kabFilter, jenisFilter, search]);

  const fmt = (n: number) => n.toLocaleString('id-ID');

  const openKonfirmasi = (row: AnomaliSKTH) => {
    setActiveRow(row);
    setKonfirmasiVal("");
  };
  ...
  const handleYa = async () => {
    ...
    setSubmitting(true);
    try {
      await submitKonfirmAnomaliSKTH({ ...activeRow, konfirmasi: konfirmasiVal });
      toast({ title: "Konfirmasi tersimpan", description: "Konfirmasi Tersimpan" });
      setConfirmOpen(false);
      setActiveRow(null);
      await queryClient.invalidateQueries({ queryKey: ["anomaliSKTH"] });
    } catch (e: any) {
  ...
  const detailFields: { label: string; key: keyof AnomaliSKTH }[] = [
      ...
      <main className="max-w-full mx-auto px-4 py-6 space-y-4">
        <div className="flex flex-col sm:flex-row gap-3 flex-wrap">
          <div className="relative flex-1 max-w-md">
            ...
            <Input placeholder="Cari nama perusahaan/catatan..." value={search} onChange={e => setSearch(e.target.value)} className="pl-10" />
          </div>
          <div className="w-56">
